...
with open("max_clique.csv", mode="w", encoding='utf-8') as w_file:
    ...
    for graph_file in graph_list:
        ...
        MAX_CLIQUE, CLIQUE_SIZE = max_clique.max_clique_search(NODES, ADJ_MATRIX)
        end_time = time.time()
        file_writer.writerow([graph_file, round(end_time - start_time, 3), CLIQUE_SIZE, MAX_CLIQUE, "\n"])

        print("Instance:", graph_file)
        print("Time:", round(end_time - start_time, 1), "seconds")
        print("Clique size:", CLIQUE_SIZE)
        print("Clique vertices:", MAX_CLIQUE)
        print("Checker:", max_clique.check(NODES, MAX_CLIQUE, ADJ_MATRIX))
